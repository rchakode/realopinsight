...
int SvNavigator::monitor(void)
{
    ...
    void *comContext = zmq_ctx_new ();
    void *comChannel= zmq_socket(comContext, ZMQ_SUB);
    assert(comChannel);
    int rc = zmq_connect(comChannel, serverUrl.c_str());
    assert (rc == 0);
    ...
    for(check_id_it = snavStruct->check_list.begin(); check_id_it != snavStruct->check_list.end(); check_id_it++) {
        ...
        char request[MonitorBroker::MAX_MSG];
        char result[MonitorBroker::MAX_MSG];
        for(node_id_it = child_nodes_list.begin(); node_id_it != child_nodes_list.end(); node_id_it++) 	{
            ...
            memset(request, 0, MonitorBroker::MAX_MSG) ;
            memcpy(request, sid.c_str(), sid.size());
            rc = zmq_send(comChannel, request, sid.size(), 0);
            assert (rc == sid.size());

            // zmq::message_t reply ;
            // comChannel->recv(&reply) ;
            memset(result, 0, MonitorBroker::MAX_MSG) ;
            int msize = zmq_recv(comChannel, result, MonitorBroker::MAX_MSG, 0);;
            assert (msize != -1);
