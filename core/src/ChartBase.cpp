...
void ChartBase::updateSeverityInfo(void)
{
  ...
  m_severityCount[ngrt4n::Major] = m_statsData[ngrt4n::Major];
  m_severityCount[ngrt4n::Minor] = m_statsData[ngrt4n::Minor];
  m_severityCount[ngrt4n::Normal] =  m_statsData[ngrt4n::Normal];
  m_severityCount[ngrt4n::Unknown] = m_nbStatsEntries
      - (m_severityCount[ngrt4n::Critical]
      + m_severityCount[ngrt4n::Major]
      + m_severityCount[ngrt4n::Minor]
      + m_severityCount[ngrt4n::Normal]);

  m_severityRatio[ngrt4n::Unknown] = qRound((100.0 * m_severityCount[ngrt4n::Unknown])/m_nbStatsEntries);
  m_severityRatio[ngrt4n::Critical] = qRound((100.0 * m_severityCount[ngrt4n::Critical])/m_nbStatsEntries);
...
QString ChartBase::buildTooltipText(void)
{
  ...
  return QObject::tr("Normal: ")%QString::number(m_severityCount[ngrt4n::Normal])%
      ...
      %"\n"%QObject::tr("Major: ")%QString::number(m_severityCount[ngrt4n::Major])%
      "/"%QString::number(m_nbStatsEntries)%" ("%QString::number(m_severityRatio[ngrt4n::Major], 'f', 0)%"%)"
      %"\n"%QObject::tr("Critical: ")%QString::number(m_severityCount[ngrt4n::Critical])%"/"
      %QString::number(m_nbStatsEntries)%" ("%QString::number(m_severityRatio[ngrt4n::Critical], 'f', 0) %"%)"
      %"\n"%QObject::tr("Unknown: ")%QString::number(m_severityCount[ngrt4n::Unknown])%
